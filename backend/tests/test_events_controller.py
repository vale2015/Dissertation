...
@pytest.mark.parametrize(
    "error,status,expected_payload",
    [
        (
            EventsRequestError("private key"),
            400,
            {"success": False, "message": "A valid event date range is required."},
        ),
        (
            EventsConfigurationError("private key"),
            500,
            {"success": False, "message": "Local-event service is not configured."},
        ),
        (
            EventsProviderError("private key", reason="invalid_request"),
            503,
            {
                "success": False,
...
def test_events_controller_returns_sanitised_failures(
    client, caplog, error, status, expected_payload
):
